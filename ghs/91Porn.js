...
const AD_SLOT_RE =
  /^(启动页|启屏页|开屏|弹框|弹窗|浮框|悬浮广告|活动弹窗|新版图标|黑料插入|长视频夹杂|-1_null|个人中心|首页推荐顶部1|首页推荐顶部2|视频详情图标|视频混合列表|社区banner广告|社区-求片-图标)$/i;

const AD_TITLE_RE =
  /(外围品茶|皇冠体育|裸聊|抖阴|AI科技|AI脱衣|男性约炮|发情增粗|开元棋牌|新葡京|永利皇宫|英皇娱乐|PG游戏|PG电子|PG娱乐|同圈速配|性界大战|决战僵尸舞娘|可乐视频|免费海角|91男漫|抖欲男漫|男男社区|爱威奶|全网片源|Pornhub|铁粉空间|免费抖阴|少妇上门|全国约炮|澳门娱乐|TikTok成人)/i;

const AD_ITEM_RE = /\/ads\/|\/upload_01\/ads\/|\/uploads\/default\/other\/|advertise_code|advertise_location_code|ad_slot_name|url_config/i;
...
function decryptPayload(dataB64) {
  ...
  var dec = CryptoJS.AES.decrypt(params, key, {
    iv: iv,
...
function isAdItem(item) {
  ...
  var ext = String(item.url || item.link_url || "");
  if (
    ext &&
    /^https?:\/\//i.test(ext) &&
    !/irtfxy|wftohhhe|iuoqtoqr|kmcbyg/i.test(ext) &&
    (AD_TITLE_RE.test(title) || /\/hc237\/uploads\/default\/other\//i.test(String(item.img_url || "")))
  )
    return true;
  var u = String(
    item.img_url || item.url || item.link_url || item.image || item.url_str || item.icon || item.icon_new || item.url_config || ""
  );
  if (AD_ITEM_RE.test(u) && (item.advertise_code || item.ad_slot_name || item.link_url || item.url || item.url_config)) return true;
  return false;
}

function isUrlObj(v) {
  return v && typeof v === "object" && (v.encrypt_url !== undefined || v.verify_token !== undefined);
}

function copyUrlObj(src) {
  if (!isUrlObj(src)) return src;
  return {
    encrypt_url: src.encrypt_url || "",
...
function unlockMvItem(item) {
  ...
  if (
    item.play_url === undefined &&
    item.pay_url_full === undefined &&
    item.preview_url === undefined &&
    item.mv_m3u8 === undefined &&
    item.coins === undefined &&
    item.is_pay === undefined
  )
    return;

  if (isUrlObj(item.play_url) || isUrlObj(item.pay_url_full)) {
    var full = item.pay_url_full;
    var play = item.play_url;
    if (isUrlObj(full)) {
      if (!isUrlObj(play) || !play.encrypt_url) item.play_url = copyUrlObj(full);
      item.pay_url_full = copyUrlObj(full);
    }
  }

  if (item.preview_url) {
    if (!item.mv_m3u8) item.mv_m3u8 = String(item.preview_url);
    item.preview_url = "";
  }
  if (item.preview_video) item.preview_video = "";
  if (item.preview_tip !== undefined) item.preview_tip = "";

  if (item.is_pay !== undefined) item.is_pay = 1;
  if (item.isfree !== undefined) item.isfree = 1;
...
function unlockUserItem(user) {
  ...
  if (user.expired_str !== undefined) user.expired_str = "2099-12-31";
  if (user.role_type !== undefined) user.role_type = "vip";
  if (user.free_card !== undefined) user.free_card = 999;
}
...
function capNotify(link, raw, priority) {
  if (capIsLongHost(link) || capIsLongHost(raw)) return;
  if (/\.m3u8/i.test(link) && !capIsPlayableHost(link) && !capIsPlayableHost(raw)) return;
...
function stripAds(node) {
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    var v = node[k];
    if (AD_KEY_RE.test(k)) {
      if (k === "pop_conf" && v && typeof v === "object") {
        var pk = Object.keys(v);
        for (var pi = 0; pi < pk.length; pi++) {
          if (typeof v[pk[pi]] === "string") v[pk[pi]] = "";
        }
        ...
        stripAds(v);
      } else if (k === "ad_play" && v && typeof v === "object") {
        node[k] = {};
      } else {
    ...
    if ((k === "top" || k === "bottom") && Array.isArray(v)) {
      for (var n = v.length - 1; n >= 0; n--) {
        if (isAdItem(v[n])) v.splice(n, 1);
        else stripAds(v[n]);
      }
      continue;
    }
