...
^https?:\/\/[^\/]+\/api\.php\/api\/ url script-response-body https://raw.githubusercontent.com/89996462/Quantumult-X/main/ghs/xhpad.js
...
^https?:\/\/new\.jfxgtd\.cn\/hc237\/uploads\/default\/other\/ - reject

[mitm]

hostname = api1.zjjjfbyo.cc, api3.zjjjfbyo.cc, wapi.zjjjfbyo.cc, *.zjjjfbyo.cc, p4.zkcrhculx.cc, *.zkcrhculx.cc, new.jfxgtd.cn, *.jfxgtd.cn, tp2.jfxgtd.cn, tp3.jfxgtd.cn
...
const AES_KEY = "2acf7e91e9864673";
...
function stripAds(node) {
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    if (k === "info" && v && typeof v === "object" && Array.isArray(v.topbanner)) {
      v.topbanner = [];
    }
    if (k === "list" && Array.isArray(v)) {
      if (v.length && isAdItem(v[0])) {
        node[k] = [];
      } else {
        for (var li = v.length - 1; li >= 0; li--) {
          if (isAdItem(v[li])) v.splice(li, 1);
        }
      }
      continue;
    }
    if (k === "value" && Array.isArray(v)) {
      ...
      for (var vi = 0; vi < v.length; vi++) {
        if (!isAdItem(v[vi])) keptValue.push(v[vi]);
      }
      node[k] = keptValue;
      continue;
    }
...
function processBody(body) {
  ...
  if (!/^[A-Za-z0-9+/=]{16,}$/.test(wrapper.data)) return null;
  try {
