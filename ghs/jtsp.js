...
function parseRsaModulus(b64) {
  var wa = CryptoJS.enc.Base64.parse(b64);
  var u8 = [];
  for (var i = 0; i < wa.sigBytes; i++)
    u8.push((wa.words[i >>> 2] >>> (24 - (i % 4) * 8)) & 255);
  for (var j = 0; j < u8.length - 5; j++) {
    if (u8[j] === 2 && u8[j + 1] === 130 && u8[j + 2] === 1 && u8[j + 3] === 1 && u8[j + 4] === 0) {
      var hex = "";
      for (var k = j + 5; k < j + 133; k++) hex += ("0" + u8[k].toString(16)).slice(-2);
      return hex;
    }
  }
  return null;
...
function rsaEncrypt16(msgBytes, modulusHex) {
  var n = BigInt("0x" + modulusHex);
  var e = 65537n;
  var k = 128;
  var pad = [];
  while (pad.length < k - msgBytes.length - 3) {
    var x = (Math.random() * 254) | 0;
    pad.push(x < 1 ? 1 : x + 1);
  }
  var em = [0, 2].concat(pad, [0]).concat(msgBytes);
  var bi = 0n;
  for (var i = 0; i < em.length; i++) bi = (bi << 8n) + BigInt(em[i]);
  var c = modPow(bi, e, n);
  var out = [];
  for (var j = 0; j < k; j++) {
    out.unshift(Number(c & 255n));
    c >>= 8n;
  }
  var words = [];
  for (var w = 0; w < out.length; w += 4) {
    words.push(
      (out[w] << 24) | ((out[w + 1] || 0) << 16) | ((out[w + 2] || 0) << 8) | (out[w + 3] || 0)
    );
  }
  return CryptoJS.enc.Base64.stringify(CryptoJS.lib.WordArray.create(words, out.length));
}

function randomHex(n) {
  var s = "";
  for (var i = 0; i < n; i++) s += ("0" + ((Math.random() * 256) | 0).toString(16)).slice(-2);
  return s;
}
...
function aesDecryptResp(encHex, keyHex) {
  var ivHex = encHex.slice(0, 32);
  var ctHex = encHex.slice(32);
  var dec = CryptoJS.AES.decrypt(
    { ciphertext: waFromHex(ctHex) },
    waFromHex(keyHex),
    { iv: waFromHex(ivHex), mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 }
  );
  return CryptoJS.enc.Utf8.stringify(dec);
}

function aesEncryptResp(plain, keyHex) {
  var ivHex = randomHex(16);
  var enc = CryptoJS.AES.encrypt(plain, waFromHex(keyHex), {
    iv: waFromHex(ivHex),
...
function patchVipDeep(obj, depth) {
  ...
  if (Array.isArray(obj)) {
    for (var i = 0; i < obj.length; i++) patchVipDeep(obj[i], depth + 1);
    return;
  }
...
function patchCaptureEncrypted(wrap, url) {
  ...
  wrap.encrypt = aesEncryptResp(JSON.stringify(data), keyHex);
  return wrap;
}
