...
function buildEndataBody(obj) {
  var ts = Math.floor(Date.now() / 1000) + 60 * new Date().getTimezoneOffset();
  return JSON.stringify({
    endata: aesEncrypt(JSON.stringify(obj)),
    ents: aesEncrypt(String(ts)),
  });
}
...
function extractVideoId(body) {
  try {
    ...
    var plain = aesDecrypt(wrapper.endata || "");
    if (!plain) return null;
    return JSON.parse(plain).videoId || null;
  } catch (e) {
    return null;
  }
}
...
function apiOrigin(url) {
  ...
  return m ? m[1] : "https://a74d.uwegu.com";
}

function corsHeaders(req) {
  return {
    "Content-Type": "application/json;charset=UTF-8",
    "Access-Control-Allow-Origin": pickHeader(req.headers, "Origin") || pickHeader(req.headers, "origin") || "*",
    "Access-Control-Allow-Methods": "POST, GET, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Auth, source, did, isshortchain, sourcewebsiteurl",
  };
}
...
function stripAdsPayload(payload) {
  ...
  if (Array.isArray(payload.list)) {
    var before = payload.list.length;
    stripAdList(payload.list);
    if (payload.list.length !== before) changed = true;
  }
  if (Array.isArray(payload)) {
    for (var i = payload.length - 1; i >= 0; i--) {
      if (isAdItem(payload[i])) {
        payload.splice(i, 1);
        changed = true;
      }
    }
  }
  if (payload.info && payload.info.Layouts) {
    var before = payload.info.Layouts.length;
    stripPanelLayouts(payload.info.Layouts);
    if (payload.info.Layouts.length !== before) changed = true;
  }
  if (payload.data && Array.isArray(payload.data)) {
    for (var j = payload.data.length - 1; j >= 0; j--) {
      if (isAdItem(payload.data[j])) {
        payload.data.splice(j, 1);
        changed = true;
      }
    }
  }
...
function unlockVideoInfo(data) {
  ...
  if (data.canPrePlay === true) {
    data.canPrePlay = false;
    changed = true;
  }
  var info = data.info;
...
function clearItemVipFlag(item) {
  if (!item || typeof item !== "object") return false;
  var changed = false;
  if (item.isVip === 1) {
    item.isVip = 0;
...
function walkClearVip(node) {
  if (!node || typeof node !== "object") return false;
  var changed = false;
  if (Array.isArray(node)) {
    for (var i = 0; i < node.length; i++) changed = walkClearVip(node[i]) || changed;
    return changed;
  }
  changed = clearItemVipFlag(node) || changed;
  var keys = Object.keys(node);
  for (var j = 0; j < keys.length; j++) changed = walkClearVip(node[keys[j]]) || changed;
  return changed;
}

function unlockGatherVideo(item) {
  if (!item || typeof item !== "object") return false;
  var changed = false;
  if (item.isVip !== 0) {
    item.isVip = 0;
...
function unlockGatherDetail(data) {
  ...
  if (Array.isArray(info.videos)) {
    for (var i = 0; i < info.videos.length; i++) {
      changed = unlockGatherVideo(info.videos[i]) || changed;
    }
  }
  return changed;
}
...
function unlockGatherList(data) {
  ...
  for (var i = 0; i < data.list.length; i++) {
    if (data.list[i] && data.list[i].isBuy !== 1) {
      data.list[i].isBuy = 1;
      changed = true;
    }
  }
...
var PROFILE_NICK = "彭于晏Crack";
var PROFILE_COIN = 99999;
var PROFILE_VIP_ENDTIME = 4102415999; // 2099-12-31 23:59:59 +08:00

function unlockUserHome(data) {
  if (!data || typeof data !== "object") return false;
  var changed = false;
  if (data.user_nicename !== PROFILE_NICK) {
    data.user_nicename = PROFILE_NICK;
    changed = true;
  }
  if (Number(data.coin) !== PROFILE_COIN) {
    data.coin = PROFILE_COIN;
    changed = true;
  }
  if (data.vip && typeof data.vip === "object") {
    ...
    if (Number(data.vip.endtime) < PROFILE_VIP_ENDTIME) {
      data.vip.endtime = PROFILE_VIP_ENDTIME;
      changed = true;
    }
    if (data.vip.level !== undefined && data.vip.level < 1) {
      data.vip.level = 5;
      changed = true;
    }
  }
...
function patchByRoute(data) {
  ...
  if (!data || typeof data !== "object") return false;
  if (/\/videos\/getInfo/i.test(REQ_URL)) return unlockVideoInfo(data);
  if (/\/videos\/(?:getList|getRecommendListByVideoId|getFeatured|getHot)/i.test(REQ_URL)) {
    return walkClearVip(data);
  }
  if (/\/gather\/getDetail/i.test(REQ_URL)) return unlockGatherDetail(data);
  if (/\/gather\/(?:getListByVideoId|getList)/i.test(REQ_URL)) return unlockGatherList(data);
  if (/\/user\/getUserHome/i.test(REQ_URL)) return unlockUserHome(data);
  if (/\/videos\/getRemainingPreNum/i.test(REQ_URL)) {
    if (data.preNum !== 999) {
      data.preNum = 999;
      return true;
    }
  ...
  if (
    ...
    /\/home\/getMoreAdsList/i.test(REQ_URL) ||
    /\/panel\/(get|list)/i.test(REQ_URL)
  ) {
...
function processBody(body) {
  ...
  if (wrapper.code !== 0 && /\/videos\/getRemainingPreNum/i.test(REQ_URL)) {
    wrapper.code = 0;
    wrapper.msg = wrapper.msg || "";
    wrapper.data = { preNum: 999 };
...
async function handleGetUrlRequest() {
  var req = $request;
  var videoId = extractVideoId(req.body);
  if (!videoId) {
    $done();
    return;
  }
...
(function main() {
  ...
  if (/\/videos\/v2\/getUrl/i.test(getReqUrl())) {
    handleGetUrlRequest();
    return;
  }
  $done();
