...
const H5_AES_KEY = "TtC1C5qYKoo6XXa3R3CjnlkDBnx9qN8U";
const H5_AES_IV = "0Va0bB39B55eMh6G";
const DIVERSION_TITLE_RE =
  /裸聊|抖阴|AI科技|爱神直播|免费直播|PG电子|开元|王者棋牌|官方推荐|V88|棋牌|导流|约炮|直播/i;
const AD_KEY_RE =
  /^(ads_screen|ads_pop|floating_ads|floating_entry|bottom_float|banner|apps|app_list|recommend_apps|partner_apps|partner_grid|grid_apps|icon_grid|app_icon|hot_grid|app_ads|ad_list|ads|advertise_list|popup_ads|launch_ads|screen_ads|splash_screen|index_popup|pop_up|pop_up_ad|active_pop|pop_ads|ads_list|pop_app_ads|pop_bottom_ads|proxy_banner_1|proxy_banner_2|layer_ads)$/i;
const H5_AD_KEY_RE =
  /^(splash_screen|index_popup|pop_up|popup_application|profile_app_group|creator_center_app_group|ranking_carousel|carousel|header|footer|floating_entry|entry_popup|pause_ad|video_pause|bottom_float)$/i;
...
function parseCipherB64(dataStr) {
  var t = String(dataStr || "").trim();
...
function decryptWithKey(dataStr, aesKey, aesIv) {
  var key = CryptoJS.enc.Utf8.parse(aesKey);
  var iv = CryptoJS.enc.Utf8.parse(aesIv);
  var dec = CryptoJS.AES.decrypt(
    { ciphertext: parseCipherB64(dataStr) },
    key,
...
function encryptWithKey(plainText, aesKey, aesIv, asUrl) {
  var key = CryptoJS.enc.Utf8.parse(aesKey);
  var iv = CryptoJS.enc.Utf8.parse(aesIv);
  var out = CryptoJS.AES.encrypt(plainText, key, {
    iv: iv,
    mode: CryptoJS.mode.CBC,
    padding: CryptoJS.pad.Pkcs7,
  }).toString();
  return asUrl ? toB64Url(out) : out;
}

function decryptPayload(dataB64) {
  return decryptWithKey(dataB64, AES_KEY, AES_IV);
}

function encryptPayload(plainText) {
  return encryptWithKey(plainText, AES_KEY, AES_IV, false);
}
...
function isDiversionItem(item) {
  if (!item || typeof item !== "object") return false;
  if (item.advertise_code) return true;
  var title = String(
    item.title || item.name || item.ati || item.ad_title || item.label || ""
  );
  if (DIVERSION_TITLE_RE.test(title)) return true;
  var url = String(item.url || item.jump_url || item.ad_url || item.ad_link || "");
  if (url && /^https?:\/\//i.test(url) && !/bh68824|beabox|ewqder123/i.test(url)) {
    return true;
  }
  return false;
}

function stripDiversionItems(node) {
  if (Array.isArray(node)) {
    for (var i = node.length - 1; i >= 0; i--) {
      if (isDiversionItem(node[i])) node.splice(i, 1);
      else stripDiversionItems(node[i]);
    }
    return;
  }
  if (!node || typeof node !== "object") return;
  var keys = Object.keys(node);
  for (var j = 0; j < keys.length; j++) stripDiversionItems(node[keys[j]]);
}

function emptyAppsGroup(group) {
  if (!group || typeof group !== "object") return;
  if (Array.isArray(group.apps)) group.apps = [];
}

function patchBeaboxAppAds(data) {
  if (!data || typeof data !== "object") return;
  if (data.splash_screen) data.splash_screen = {};
  if (data.index_popup) data.index_popup = [];
  emptyAppsGroup(data.popup_application);
  emptyAppsGroup(data.profile_app_group);
  emptyAppsGroup(data.creator_center_app_group);
  emptyAppsGroup(data.ranking_carousel);
}

function patchBeaboxApplicationAds(data) {
  if (!data || typeof data !== "object") return;
  data.carousel = [];
  data.header = [];
  data.footer = [];
  if (Array.isArray(data.application)) {
    for (var i = 0; i < data.application.length; i++) emptyAppsGroup(data.application[i]);
  }
}

function patchBeaboxConfig(data) {
  if (!data || typeof data !== "object") return;
  if (data.pop_up) data.pop_up = {};
}

function patchBeaboxCampaigns(data) {
  if (!data || typeof data !== "object") return;
  if (data.ui) {
    if (data.ui.entry_popup) {
      data.ui.entry_popup.enabled = false;
      data.ui.entry_popup.image_url = "";
      data.ui.entry_popup.url = "";
    }
    if (data.ui.floating_entry) {
      data.ui.floating_entry.enabled = false;
      data.ui.floating_entry.image_url = "";
      data.ui.floating_entry.url = "";
    }
  }
}

function stripBeaboxAds(node) {
  if (Array.isArray(node)) {
    for (var i = 0; i < node.length; i++) stripBeaboxAds(node[i]);
    return;
  }
  if (!node || typeof node !== "object") return;
  var keys = Object.keys(node);
  for (var j = 0; j < keys.length; j++) {
    var k = keys[j];
    var v = node[k];
    if (H5_AD_KEY_RE.test(k)) {
      node[k] = Array.isArray(v) ? [] : {};
      continue;
    }
    if (k === "apps" && Array.isArray(v)) {
      node[k] = [];
      continue;
    }
    stripBeaboxAds(v);
...
function patchBeaboxPayload(payload, reqUrl) {
  ...
  if (!data || typeof data !== "object") return;
  var url = reqUrl || "";
  if (url.indexOf("/app/ads") >= 0) patchBeaboxAppAds(data);
  else if (url.indexOf("/application/ads") >= 0) patchBeaboxApplicationAds(data);
  else if (url.indexOf("/config/data") >= 0) patchBeaboxConfig(data);
...
function patchNativePayload(payload, reqUrl) {
  ...
  if (payload.data) {
    ...
    stripDiversionItems(payload.data);
  }
  stripAdvertiseItems(payload);
  stripDiversionItems(payload);
  if (
    reqUrl &&
    reqUrl.indexOf("getADsByPosition") >= 0 &&
    payload.data &&
...
function needsH5Cipher(dataStr) {
...
function isBeaboxRequest(reqUrl) {
  return reqUrl && /ewqder123\.top/i.test(reqUrl);
}

function isNativeApiRequest(reqUrl) {
  return reqUrl && /api\.php\/api\//i.test(reqUrl);
}

function processNativeBody(body, reqUrl) {
  ...
  if (wrapper.timestamp === undefined && wrapper.sign === undefined) return null;
  try {
    var plain = decryptPayload(wrapper.data);
    var payload = JSON.parse(plain);
    patchNativePayload(payload, reqUrl || "");
...
function processBeaboxBody(body, reqUrl) {
  if (!body) return null;
  var wrapper;
  try {
    wrapper = JSON.parse(body);
  ...
  try {
    if (wrapper.data !== undefined && needsH5Cipher(wrapper.data)) {
      var plain = decryptWithKey(wrapper.data, H5_AES_KEY, H5_AES_IV);
      var payload = JSON.parse(plain);
      patchBeaboxPayload(payload, reqUrl || "");
      wrapper.data = encryptWithKey(JSON.stringify(payload), H5_AES_KEY, H5_AES_IV, true);
      return JSON.stringify(wrapper);
    }
    if (wrapper.status !== undefined) {
      patchBeaboxPayload(wrapper, reqUrl || "");
      return JSON.stringify(wrapper);
    }
  } catch (e) {
    return null;
...
function processBody(body, reqUrl) {
  if (isBeaboxRequest(reqUrl)) return processBeaboxBody(body, reqUrl);
  if (isNativeApiRequest(reqUrl)) return processNativeBody(body, reqUrl);
