...
const AES_KEY = "tJqkoiJXvD7UCU3i";
...
function aesDecrypt(b64) {
  return CryptoJS.AES.decrypt(String(b64), KEY, AES_OPT).toString(CryptoJS.enc.Utf8);
}
...
function calcSign(data, errcode, timestamp) {
  var s =
    "data=" +
    data +
    "&errcode=" +
    ...
    timestamp +
    SIGN_SALT;
  return CryptoJS.MD5(CryptoJS.enc.Utf8.parse(CryptoJS.SHA256(s).toString())).toString();
}

function normalizePlayUrl(url) {
  if (!url) return "";
  var link = String(url);
  link = link.replace(/-\d*120play\./i, function (m) {
    return m.replace(/120play/i, "long");
  });
  link = link.replace(/120play\./i, "long.");
  link = link.replace(/([?&])seconds=\d+/gi, "$1");
  link = link.replace(/[?&]$/, "");
  if (link.indexOf("via_m=") < 0) {
...
function isAdItem(item) {
  ...
  if (item.advertise_code !== undefined && item.advertise_code !== "") return true;
  if (item.ad_slot_name !== undefined && item.ad_slot_name !== "") return true;
  if (item.ad_type !== undefined && item.ad_type !== null && item.ad_type !== "") return true;
  var name = String(item.title || item.name || item.short_name || "");
  ...
  // 导航 mid_style_* / bot_style_* 的 icon 也走 /upload_01/ads/ 路径，不能按 icon 判广告
  if (/^https?:\/\//i.test(String(item.url || "")) && AD_TITLE_RE.test(String(item.url))) return true;
  return false;
}
...
function emptyAdKey(key, val) {
  if (!AD_EMPTY_KEYS[key]) return false;
...
function unlockVideoNode(node) {
  ...
  if (node.m3u8_full !== undefined && node.m3u8_full) {
    ...
    if (m3u8 !== node.m3u8_full) {
      node.m3u8_full = m3u8;
      changed = true;
    }
  }
  if (node.is_pay !== undefined && Number(node.is_pay) !== 0) {
    node.is_pay = 0;
    changed = true;
  }
  if (node.is_free !== undefined && Number(node.is_free) !== 1) {
    node.is_free = 1;
    changed = true;
  }
...
function unlockUserNode(node) {
  ...
  if (node.vip_level !== undefined && Number(node.vip_level) < 1) {
    node.vip_level = 1;
    changed = true;
  }
  if (node.coins !== undefined && Number(node.coins) < PROFILE_COIN) {
    node.coins = PROFILE_COIN;
    changed = true;
  }
  if (node.g_coins !== undefined && Number(node.g_coins) < PROFILE_COIN) {
    node.g_coins = String(PROFILE_COIN);
    changed = true;
  }
  if (node.expired_at !== undefined && Number(node.expired_at) < PROFILE_VIP_END) {
...
function walkPatch(node, depth, mode) {
  ...
  changed = unlockUserNode(node) || changed;
  var keys = Object.keys(node);
  for (var k = 0; k < keys.length; k++) {
    var key = keys[k];
    var val = node[key];
    if (emptyAdKey(key, val)) {
      changed = true;
      continue;
    }
...
function processBody(body) {
  if (!body || body.indexOf('"data"') < 0 || body.indexOf('"errcode"') < 0) return null;
  var outer;
  try {
  ...
  if (!outer || typeof outer.data !== "string" || !outer.data) return null;
  var plain;
  try {
    plain = aesDecrypt(outer.data);
...
(function main() {
  ...
  if (newBody) $done({ body: newBody, headers: $response.headers });
  else $done();
})();
