...
function hasDKey() {
  return !!(AES_KEY_D && AES_KEY_D.length >= 16);
}

function decryptPayloadD(dataB64) {
  var key = CryptoJS.enc.Utf8.parse(AES_KEY_D);
  var iv = CryptoJS.enc.Utf8.parse(AES_IV_D);
...
function encryptPayloadD(plainText) {
  var key = CryptoJS.enc.Utf8.parse(AES_KEY_D);
  var iv = CryptoJS.enc.Utf8.parse(AES_IV_D);
  return CryptoJS.AES.encrypt(plainText, key, {
    iv: iv,
...
function isAdResourceItem(item) {
  ...
  if (item.module_types === 4) return true;
  if (
    item.position_id &&
    typeof item.img_path === "string" &&
    /\/uploads\/images\/ads\//i.test(item.img_path)
  ) {
    return true;
  }
  var u =
    ...
    item.img_path ||
    "";
  if (typeof u === "string" && (/\/uploads\/images\/ads\//i.test(u) || /\/ads\//i.test(u))) {
    return true;
  }
  if (item.info && typeof item.info === "object") {
    return isAdResourceItem(item.info);
  }
  return false;
}
...
function patchHomeConfig(data) {
  ...
  data.pop_app_ads = [];
  data.pop_bottom_ads = [];
  if (data.notice && typeof data.notice === "object") data.notice = {};
}

function patchHomeIndex(data) {
  if (!data || typeof data !== "object") return;
  patchHomeIndexModules(data.info);
  patchHomeIndexModules(data.list);
}

function patchHomeIndexModules(modules) {
  if (!Array.isArray(modules)) return;
  for (var i = 0; i < modules.length; i++) {
    var mod = modules[i];
    if (!mod || typeof mod !== "object") continue;
    if (Array.isArray(mod.ads)) mod.ads = [];
    else if (mod.ads && typeof mod.ads === "object") mod.ads = {};
    if (!Array.isArray(mod.list)) continue;
    for (var j = mod.list.length - 1; j >= 0; j--) {
      if (isAdResourceItem(mod.list[j])) mod.list.splice(j, 1);
    }
  }
}

function clearResponseSignature(payload) {
  if (payload && typeof payload === "object" && payload.s !== undefined) {
    delete payload.s;
  }
}
...
function patchPayloadByUrl(payload, reqUrl) {
  ...
  if (u.indexOf("/api/rich_new/") >= 0) {
    if (payload.data && typeof payload.data === "object") {
      if (Array.isArray(payload.data.ads)) payload.data.ads = [];
      else if (payload.data.ads && typeof payload.data.ads === "object") payload.data.ads = {};
  ...
  if (u.indexOf("/api/popup/") >= 0) {
    stripAds(payload);
    if (payload.data && typeof payload.data === "object") {
      if (Array.isArray(payload.data.recommend_list)) payload.data.recommend_list = [];
      if (Array.isArray(payload.data.ads)) payload.data.ads = [];
      else if (payload.data.ads && typeof payload.data.ads === "object") payload.data.ads = {};
    }
  ...
  if (u.indexOf("/api/v1/homeIndex") >= 0) {
    if (payload.data && typeof payload.data === "object") patchHomeIndex(payload.data);
    stripAds(payload);
...
function patchPayload(payload, reqUrl) {
  ...
  if (payload.data) {
    ...
    if (reqUrl && reqUrl.indexOf("/api/v1/homeIndex") >= 0) patchHomeIndex(d);
    stripAds(d);
    stripBannerFields(d);
