  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    if (AD_KEY_RE.test(k)) {
      if (k === "ads" && v && typeof v === "object" && !Array.isArray(v)) {
        ...
        node[k] = 0;
        continue;
      } else if (k === "bury_point" && v && typeof v === "object") {
        if (v.is_report_advertising !== undefined) v.is_report_advertising = 0;
        if (v.is_report_ad_impression !== undefined) v.is_report_ad_impression = 0;
        if (v.is_report_ad_click !== undefined) v.is_report_ad_click = 0;
        stripAds(v);
...
function normalizePlayUrl(url) {
  if (!url) return "";
  var u = String(url);
  u = u.replace(/([a-zA-Z0-9_-]+)-120play\./g, "$1-long.");
  u = u.replace(/:\/\/120play\./g, "://long.");
  u = u.replace(/&seconds=\d+/g, "").replace(/\?seconds=\d+&/g, "?").replace(/\?seconds=\d+$/g, "");
...
function unlockMember(node) {
  ...
  if (node.dark_privilege !== undefined) node.dark_privilege = true;
  if (node.data && typeof node.data === "object") {
    var u = node.data;
    if (u.money !== undefined) u.money = 99999;
    if (u.score !== undefined) u.score = 99999;
    if (u.level !== undefined) u.level = 99;
    if (u.exp !== undefined) u.exp = 99999;
...
function unlockComic(node) {
  if (!node || typeof node !== "object") return;
  if (node.is_free !== undefined) node.is_free = 1;
  if (node.isfree !== undefined) node.isfree = 1;
  if (node.view_money !== undefined) node.view_money = 0;
  if (node.download_money !== undefined) node.download_money = 0;
  if (node.obtained !== undefined) node.obtained = 1;
  if (node.is_pay !== undefined) node.is_pay = 0;
  if (node.need_pay !== undefined) node.need_pay = 0;
  if (node.can_read !== undefined) node.can_read = 1;
  if (node.unlock_coins !== undefined) node.unlock_coins = 0;
  if (node.discount_before_vip_unlock_coins !== undefined) {
    node.discount_before_vip_unlock_coins = 0;
  }
  if (node.is_lock !== undefined) node.is_lock = 0;
  var keys = Object.keys(node);
  for (var i = 0; i < keys.length; i++) unlockComic(node[keys[i]]);
}

function unlockVideo(node) {
  if (!node || typeof node !== "object") return;
  var full = "";
  if (node.play_url) full = normalizePlayUrl(node.play_url);
  else if (node.preview) full = normalizePlayUrl(node.preview);
  else if (node.preview_video) full = normalizePlayUrl(node.preview_video);
  if (full) {
    node.play_url = full;
    node.preview = full;
    if (node.source_720 !== undefined) node.source_720 = full;
    if (node.source_480 !== undefined) node.source_480 = full;
    if (node.source_1080 !== undefined) node.source_1080 = full;
    if (node.source_240 !== undefined) node.source_240 = full;
    if (node.source_origin_str !== undefined) node.source_origin_str = full;
    if (node.preview_video !== undefined) node.preview_video = "";
    if (node.preview_tip !== undefined) node.preview_tip = "";
  }
  if (node.isfree !== undefined) node.isfree = 1;
  if (node.is_free !== undefined) node.is_free = 1;
  if (node.is_pay !== undefined) node.is_pay = 0;
  if (node.web_free !== undefined) node.web_free = 1;
  if (node.coins !== undefined) node.coins = 0;
  if (node.discount_coins !== undefined) node.discount_coins = 0;
  var keys = Object.keys(node);
  for (var i = 0; i < keys.length; i++) unlockVideo(node[keys[i]]);
}

function filterAdArrays(node) {
  if (Array.isArray(node)) {
    for (var i = node.length - 1; i >= 0; i--) {
      if (isAdItem(node[i])) node.splice(i, 1);
      else filterAdArrays(node[i]);
    }
    return;
  }
  if (!node || typeof node !== "object") return;
...
function fixHeaders(headers, bodyStr) {
  ...
  var len = 0;
  for (var j = 0; j < bodyStr.length; j++) {
    var c = bodyStr.charCodeAt(j);
    len += c <= 127 ? 1 : c <= 2047 ? 2 : 3;
  }
