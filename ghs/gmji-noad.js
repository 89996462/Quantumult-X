...
function isAdItem(item) {
  ...
  if (AD_ITEM_RE.test(u) && (item.ads_code || item.ad_slot_name || item.link_url || item.url || item.url_config)) return true;
  return false;
}

function fmtExpire() {
  var d = new Date(Date.now() + 86400 * 365 * 1000);
  var pad = function (n) {
    return n < 10 ? "0" + n : "" + n;
  };
  return (
...
function buildFullPlayUrl(preview) {
  if (!preview) return "";
  return String(preview).replace(/-10\.m3u8/gi, ".m3u8");
}

function unlockMvItem(item) {
  if (!item || typeof item !== "object") return;
  if (item.preview_url === undefined && item.mv_m3u8 === undefined && item.coins === undefined) return;
  if (item.preview_url) {
    if (!item.mv_m3u8) item.mv_m3u8 = buildFullPlayUrl(item.preview_url);
    item.preview_url = "";
  } else if (item.mv_m3u8 && /-10\.m3u8/i.test(item.mv_m3u8)) {
    item.mv_m3u8 = buildFullPlayUrl(item.mv_m3u8);
  }
...
function unlockUserItem(user) {
  ...
  if (user.money !== undefined) user.money = 99999;
  if (user.points !== undefined) user.points = 99999;
  if (user.free_view_cnt !== undefined) user.free_view_cnt = 999;
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    if (AD_KEY_RE.test(k)) {
      if (k === "notice" && v && typeof v === "object") {
        node[k] = {};
      } else if (k === "event_info" && v && typeof v === "object") {
        ["is_report_ad_show", "is_report_ad_click", "is_report_ad_action", "is_report_page_click", "is_report_page_load", "is_report_nav", "is_report_app_install"].forEach(function (f) {
          if (v[f] !== undefined) v[f] = 0;
        });
        stripAds(v);
      } else if (k === "versionMsg") {
        node[k] = Array.isArray(v) ? [] : typeof v === "object" && v ? {} : "";
