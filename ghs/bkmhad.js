...
function aesDecrypt(cipher) {
  try {
    return CryptoJS.AES.decrypt(String(cipher).trim(), KEY, AES_OPT).toString(CryptoJS.enc.Utf8);
  } catch (e) {
    return "";
  }
}
...
function aesEncrypt(plain) {
  return CryptoJS.AES.encrypt(String(plain), KEY, AES_OPT).toString();
}
...
function isAdItem(item) {
  ...
  if (String(item.type || "").toLowerCase() === "ad") return true;
  return false;
}

function filterAdList(arr) {
...
function clearArrayField(obj, key) {
  if (!obj || !Array.isArray(obj[key]) || !obj[key].length) return false;
  obj[key] = [];
  return true;
}

function stripLayerAds(layer) {
  ...
  for (var i = layer.length - 1; i >= 0; i--) {
    ...
    if (t === "ad" || t === "apps" || isAdItem(item.data)) {
      layer.splice(i, 1);
      changed = true;
    }
  }
...
function stripAdsData(data) {
  if (!data || typeof data !== "object") return false;
  var changed = false;
  changed = filterAdList(data.banner) || changed;
  changed = filterAdList(data.apps) || changed;
  changed = clearArrayField(data, "ads") || changed;
  changed = clearArrayField(data, "bottom_ad") || changed;
  if (stripLayerAds(data.layer)) changed = true;
  if (data.ad_auto_jump !== undefined && String(data.ad_auto_jump) !== "n") {
    data.ad_auto_jump = "n";
    changed = true;
  }
  if (data.ad_show_time !== undefined && String(data.ad_show_time) !== "0") {
    data.ad_show_time = "0";
    changed = true;
  }
  if (data.show_ad_method !== undefined && String(data.show_ad_method) !== "0") {
    data.show_ad_method = "0";
    changed = true;
  }
  if (Array.isArray(data.block)) {
    for (var b = 0; b < data.block.length; b++) {
      var block = data.block[b];
      if (!block || typeof block !== "object") continue;
      if (clearArrayField(block, "ads")) changed = true;
      if (filterAdList(block.items)) changed = true;
    }
  }
  return changed;
}
...
function unlockChapterItem(item) {
  if (!item || typeof item !== "object") return false;
  var changed = false;
  if (String(item.can_view) !== "y") {
    item.can_view = "y";
    changed = true;
  }
  if (String(item.type) === "vip" || String(item.type) === "money") {
    item.type = "free";
    changed = true;
  }
  if (item.money !== undefined && String(item.money) !== "0") {
    item.money = "0";
    changed = true;
  }
  return changed;
...
function unlockContentDetail(data) {
  if (!data || typeof data !== "object") return false;
  var changed = false;
  changed = stripAdsData(data) || changed;
  if (data.no_vip_error) {
    data.no_vip_error = "";
    changed = true;
  }
  if (String(data.last_chapter_can_view) !== "y") {
    data.last_chapter_can_view = "y";
    changed = true;
  }
  if (data.last_chapter_type && String(data.last_chapter_type) !== "free") {
    data.last_chapter_type = "free";
    changed = true;
  }
  if (Array.isArray(data.chapter)) {
    for (var i = 0; i < data.chapter.length; i++) {
      changed = unlockChapterItem(data.chapter[i]) || changed;
    }
  }
...
function normalizePlayPath(url) {
...
function unlockPlayLinks(data) {
  if (!data || !Array.isArray(data.play_links)) return false;
  var changed = false;
  var captured = "";
  for (var i = 0; i < data.play_links.length; i++) {
    var line = data.play_links[i];
    if (!line || typeof line !== "object") continue;
    var full = normalizePlayPath(line.m3u8_url);
    var preview = normalizePlayPath(line.preview_m3u8_url);
    if (!full && preview) {
      line.m3u8_url = preview;
      full = preview;
      changed = true;
    }
...
function unlockBuyResponse(data) {
  if (!data || typeof data !== "object") return false;
  var changed = false;
  if (data.can_view !== undefined && String(data.can_view) !== "y") {
    data.can_view = "y";
    changed = true;
  }
  if (data.status !== undefined && String(data.status) !== "y") {
    data.status = "y";
    changed = true;
  }
  return changed;
...
function patchData(data, reqUrl) {
...
function processBody(body) {
  ...
  var plain = aesDecrypt(body);
  if (!plain) return null;
  var wrapper;
  try {
    wrapper = JSON.parse(plain);
  } catch (e) {
    return null;
  }
  if (!wrapper || wrapper.data === undefined) return null;
  var changed = patchData(wrapper.data, getReqUrl());
  if (!changed) return null;
  return aesEncrypt(JSON.stringify(wrapper));
}
