...
# 特别说明：捕获成功后，点击通知即可观看
# 脚本作者：彭于晏💞
# 更新时间：2026-06-03（抓包 2026-06-03-144230 / v9 elements.value 16宫格）
...
^https?:\/\/[^\/]+\/(?:api|pwa)\.php url script-response-body https://raw.githubusercontent.com/89996462/Quantumult-X/main/ghs/91avad.js
...
hostname = *.yeuxqifz.cc, *.xoceiqxo.cc, *.ceogberj.cc, *.caanrrim.cc, *.iajckz.cn, *.xmwemv.cn, *.bwkhawkdv.cc, p4.bxlpxkmvh.com, *.bxlpxkmvh.com, tp6.xmwemv.cn, tp7.xmwemv.cn, 120play.*.cn, long.*.cn, *.wpyxbxt.cn, h5play.*.com, *.fipxor.cn, ap.dc-report.cc, api-dc-prod-002.cyou, api-dc2-prod-02.cyou
...
// 新版 _ver=v0（main.dart.js bza/bz1，POST /api.php）
const AES_KEY_V0 = "f1b97d6206b8ccd91b75f90d97cb11ea";
const AES_IV_V0 = "bbb5e8de80737ee0";
const SIGN_SALT_V0 = "f479a05f2c1b6688aed5c4a5213fcdfe";
...
const GRID_ENTRY_RE =
  /裸聊|抖阴|AI科技|AI\s*科技|16宫格|宫格导流|吃瓜|污漫|次元|约炮|菠菜|新版图标|首页图标|PG棋牌|PG电子|PG发|青楼|春药|永利|外围品茶|91PORN|点我福利|Hot\s*girls|电子棋牌|大发娱乐|澳门娱乐|澳门新|新葡京|葡萄京|皇宫|饥渴欲女|品茶|迷奸|校园直播|杏吧|免费看片|潘多拉|送1888|在线约炮/i;
...
function decryptPayloadB64V0(dataB64) {
  var key = CryptoJS.enc.Utf8.parse(AES_KEY_V0);
  var params = CryptoJS.lib.CipherParams.create({
    ciphertext: CryptoJS.enc.Base64.parse(dataB64),
  });
  var dec = CryptoJS.AES.decrypt(params, key, aesOptionsCbc(AES_IV_V0));
  return dec.toString(CryptoJS.enc.Utf8);
}
...
function encryptPayloadB64V0(plainText) {
  var key = CryptoJS.enc.Utf8.parse(AES_KEY_V0);
  var enc = CryptoJS.AES.encrypt(
    CryptoJS.enc.Utf8.parse(plainText),
    key,
    aesOptionsCbc(AES_IV_V0)
  );
  return CryptoJS.enc.Base64.stringify(enc.ciphertext);
}
...
function isHexCipher(data) {
  return /^FF07BB[0-9A-F]+$/i.test(data);
}

function isB64Cipher(data) {
  return /^[A-Za-z0-9+/=_-]+$/.test(data) && data.length > 24;
}
...
function isSplashScreen(obj) {
  if (!obj || typeof obj !== "object" || Array.isArray(obj)) return false;
  if (/启动页|开屏|splash/i.test(String(obj.ad_slot_name || ""))) return true;
  return isNoticeAd(obj);
}

function stripSplashScreen(data) {
  if (!data || typeof data !== "object") return;
  if (isSplashScreen(data.screen)) data.screen = {};
}
...
function stripPageAds(data) {
  ...
  filterAdArray(data.pop_ads);
  filterAdArray(data.value);
  if (Array.isArray(data.elements)) {
    for (var ei = 0; ei < data.elements.length; ei++) {
      var el = data.elements[ei];
      if (!el || typeof el !== "object") continue;
      filterAdArray(el.value);
      filterAdArray(el.banner);
      filterAdArray(el.list);
      stripPartNested(el);
    }
  }
...
function isGridDiversionArray(arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    if (isInjectedListAd(arr[i]) || isAdItem(arr[i])) adHit++;
  }
  if (adHit >= 6) return true;
  if (arr.length >= 9 && arr.length <= 32 && hit >= 3) return true;
  return false;
}
...
function stripAds(node) {
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    if (k === "pop_ads" && Array.isArray(v)) {
      filterAdArray(v);
      continue;
    }
    if (k === "value" && Array.isArray(v)) {
      if (isGridDiversionArray(v)) node[k] = [];
      else filterAdArray(v);
      continue;
    }
    if (k === "elements" && Array.isArray(v)) {
      for (var ei = 0; ei < v.length; ei++) {
        stripAds(v[ei]);
      }
      continue;
    }
...
function processBody(body) {
  ...
  var useHex = isHexCipher(wrapper.data);
  ...
  try {
    ...
    if (useHex) {
      ...
      plain = decryptPayloadB64V0(wrapper.data);
      if (isValidJsonPlain(plain)) mode = "v0";
      else {
    ...
    } else if (payload.data && (payload.data.pop_ads || payload.data.screen)) {
      stripHomeConfig(payload.data);
    }
    stripPageAds(inner);
    stripSplashScreen(inner);
    stripAds(inner);
    ...
    wrapper.data =
      mode === "hex"
        ...
        : mode === "v0"
          ? encryptPayloadB64V0(JSON.stringify(payload))
    ...
    if (mode === "v3") {
      ...
      wrapper.sign = calcSignSorted(wrapper, SIGN_SALT_V0);
    } else {
...
if (!AD_API_PATH_RE.test(reqUrl.split("?")[0])) {
  ...
  var body = normalizeResponseBody($response.body);
  ...
  if (newBody) {
    ...
    $done({ body: newBody, headers: hdrs });
  } else {
    $done();
  }
}
