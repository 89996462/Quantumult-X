...
var FEED_PATCH_URL_RE = /\/api\/tabnew\/list_construct/i;
...
function ensureCryptoJS() {
  ...
  if (g.__xlspadCryptoJS) {
    CryptoJS = g.__xlspadCryptoJS;
    return;
  }
(function () {
  var g = typeof globalThis !== "undefined" ? globalThis : this;
...
function decryptPayload(dataHex) {
  ...
  var dec = CryptoJS.AES.decrypt(b64, CryptoJS.enc.Utf8.parse(AES_KEY), {
...
function encryptPayload(plainText) {
  var enc = CryptoJS.AES.encrypt(CryptoJS.enc.Utf8.parse(plainText), CryptoJS.enc.Utf8.parse(AES_KEY), {
    iv: CryptoJS.enc.Utf8.parse(AES_IV),
    mode: CryptoJS.mode.CFB,
    padding: CryptoJS.pad.NoPadding,
  });
...
function isAdItem(item) {
  ...
  if (item.icon !== undefined || item.icon_new !== undefined) {
    if (item.advertise_code || item.advertise_location_code || item.ad_slot_name) return true;
    if (item.title && GRID_APP_TITLES.test(String(item.title).trim())) return true;
    return false;
  }
  if (item.advertise_code || item.advertise_location_code || item.ad_slot_name || item.ad_type) return true;
  if (typeof item.position === "number" && item.position >= 2001 && item.position < 2100) return true;
  var url = String(item.img_url || item.url || item.link_url || "");
  if (/\/upload_01\/ads\//i.test(url) || /\/hc237\/uploads\/default\/other\//i.test(url) || /\/new\/ads\//i.test(url)) return true;
  if (item.title && GRID_APP_TITLES.test(String(item.title).trim())) return true;
  if (item.ad_slot_name && /九宫格|应用中心图标/i.test(String(item.ad_slot_name))) return true;
  return false;
}
...
function stripMidStyleAds(node) {
  if (!node || typeof node !== "object") return;
  ["mid_style_category", "mid_style_recommend"].forEach(function (key) {
...
function isFeedDataValid(data) {
  if (!data || typeof data !== "object") return false;
  if (!Array.isArray(data.bot_style_one) || data.bot_style_one.length === 0) return false;
  return true;
}

function feedNeedsPatch(data) {
  if (!data || typeof data !== "object") return false;
  var i, item, list;
  if (Array.isArray(data.banner)) {
    for (i = 0; i < data.banner.length; i++) {
      if (isAdItem(data.banner[i])) return true;
    }
  }
  if (Array.isArray(data.list_ads) && data.list_ads.length > 0) return true;
  if (Array.isArray(data.floating_ads) && data.floating_ads.length > 0) return true;
  ["mid_style_category", "mid_style_recommend"].forEach(function (key) {
    list = data[key];
    if (!Array.isArray(list)) return;
    for (i = 0; i < list.length; i++) {
      if (isMidStyleAd(list[i])) return true;
    }
  });
  return false;
}
...
function stripAds(node, depth) {
  ...
  if (Array.isArray(node)) {
    for (var i = 0; i < node.length; i++) stripAds(node[i], depth + 1);
    return;
  }
  if (!node || typeof node !== "object") return;
...
function patchPayload(payload, reqUrl) {
  ...
  if (root.data) {
    stripAds(root.data);
    stripAdvertiseItems(root.data);
    stripMidStyleAds(root.data);
    if (Array.isArray(root.data.apps)) root.data.apps = [];
    if (Array.isArray(root.data.ads_screen)) root.data.ads_screen = [];
    if (Array.isArray(root.data.pop_ads_v2)) root.data.pop_ads_v2 = [];
    if (Array.isArray(root.data.floating_ads)) root.data.floating_ads = [];
    if (Array.isArray(root.data.list_ads)) root.data.list_ads = [];
    if (Array.isArray(root.data.banner)) {
      root.data.banner = root.data.banner.filter(function (item) {
        return !isAdItem(item);
      });
    }
    if (Array.isArray(root.data.banners)) {
      root.data.banners = root.data.banners.filter(function (item) {
        return !isAdItem(item);
      });
    }
  }
  stripAdvertiseItems(root);
  stripMidStyleAds(root);
  if (reqUrl && (reqUrl.indexOf("getOpenAdsAndVersion") >= 0 || reqUrl.indexOf("getConfig") >= 0) && root.data) {
    root.data.ads = null;
    root.data.screen_ads = [];
    root.data.pop_ads = [];
    root.data.pop_ads_v2 = [];
    root.data.layer_ads = [];
    root.data.ads_screen = [];
    root.data.floating_ads = [];
    root.data.list_ads = [];
    root.data.apps = [];
    root.data.notice = null;
  }
  if (reqUrl && reqUrl.indexOf("getADsByPosition") >= 0 && root.data && Array.isArray(root.data)) {
    root.data = [];
  }
}

function parseInnerPayload(plain) {
  var parsed = JSON.parse(plain);
  if (typeof parsed === "string") parsed = JSON.parse(parsed);
  return parsed;
}

function stringifyInnerPayload(payload) {
...
function processBody(body, reqUrl) {
  ...
  var isFeed = FEED_PATCH_URL_RE.test(reqUrl || "");
  try {
    var plain = decryptPayload(wrapper.data);
    var payload = parseInnerPayload(plain);
    if (isFeed && payload.data && !feedNeedsPatch(payload.data)) return null;
    var snapshot = JSON.stringify(payload);
    patchPayload(payload, reqUrl || "");
    if (isFeed && payload.data && !isFeedDataValid(payload.data)) return null;
